...
pub async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut tokens = Arc::new(db_client.get_tokens(config.patreon_client_id.clone()).await?);
...
async fn handle_refresh(tokens: &database::Tokens, config: &Config, db_client: &Database) -> Result<database::Tokens, Box<dyn Error>> {
    let new_tokens = oauth::refresh_tokens(tokens.refresh_token.clone(), config.patreon_client_id.clone(), config.patreon_client_secret.clone()).await?;

    let tokens = database::Tokens::new(new_tokens.access_token, new_tokens.refresh_token, current_time_seconds() + new_tokens.expires_in);
    db_client.update_tokens(config.patreon_client_id.clone(), &tokens).await?;
